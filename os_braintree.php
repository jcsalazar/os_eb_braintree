...
class os_braintree extends os_payment
{
	...
	function processPayment($row, $data)
	{
		...
		->innerJoin('#__eb_events AS b ON a.user_id = b.created_by')			
		->where('b.id=' . $row->event_id)	
		->where(' (profile_key ="profile.aboutme")');		
		$db->setQuery($query);				

		$rowuserprofile = $db->loadObject();			
		
		$result = Braintree_Transaction::sale(array(
		    'merchantAccountId' => str_replace('"', '', $rowuserprofile->profile_value),
			'amount'     => round($data['amount'], 2),
			'orderId'    => $row->id,
			'serviceFeeAmount' => $mgmtfee,
			'creditCard' => array(
